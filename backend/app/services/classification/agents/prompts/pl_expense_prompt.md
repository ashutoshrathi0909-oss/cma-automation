...
This is an industry variant driven by EACH ITEM's industry_type field (not the batch-level field — items may come from mixed-industry batches). Manufacturing: employee costs -> R45 (Wages, under Manufacturing Expenses). Trading/Services/Retail: employee costs -> R67 (Salary and staff expenses, under Admin). Sub-items like "Contribution to EPF", "Gratuity", "Staff Welfare", "Employee Welfare", "Workmen Welfare", "Staff Amenities" follow the same industry split. Exception: "Director Remuneration" ALWAYS -> R73 regardless of industry.

**Staff Welfare rule (explicit, CA 2026-04-15):** "Staff Welfare" / "Employee Welfare" / "Workmen Welfare" / "Staff Amenities" are EMPLOYEE COSTS. **STEP 1: Read the ITEM-LEVEL industry_type field FIRST (each item now carries its own industry_type). STEP 2: THEN apply the row.** If item's industry_type is absent, fall back to the batch-level industry_type. Do not skip step 1.
...
7. **Staff-Welfare item-level industry guard (CA 2026-04-15, final):** For any item whose description contains "Staff Welfare" / "Employee Welfare" / "Workmen Welfare" / "Staff Amenities", check the ITEM-LEVEL industry_type field (each item carries its own). If item's industry_type == "manufacturing" AND classified to R67/R71, re-classify to R45. If item's industry_type in {"trading","services","retail"} AND classified to R45, re-classify to R67. The item-level field is authoritative because batches can be mixed-industry (retail items may appear in a manufacturing-majority batch).
