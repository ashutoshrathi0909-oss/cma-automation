#!/usr/bin/env python3
"""
Dynamic Air Engineering accuracy benchmark — Phase 1 only.

Reads ground truth from CMA_Ground_Truth_v1/companies/Dynamic_Air/ground_truth_normalized.json,
classifies each item via ScopedClassifier, and saves summary + wrong entries.

Run: docker compose exec worker python /app/test-results/run_accuracy_dynamic_air.py
"""
...
# Ensure /app is on the path so `app.*` imports work from this subdirectory
sys.path.insert(0, "/app")

# ─── Paths ────────────────────────────────────────────────────────────────────
GT_FILE = Path("/app/CMA_Ground_Truth_v1/companies/Dynamic_Air/ground_truth_normalized.json")
OUTPUT_DIR = Path("/app/test-results")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_FILE = OUTPUT_DIR / "Dynamic_Air_accuracy_summary.json"
WRONG_FILE = OUTPUT_DIR / "Dynamic_Air_wrong_entries.json"
...
print("=" * 60)
print("Dynamic Air Engineering Accuracy Benchmark — Phase 1")
print("=" * 60)
...
print("RESULTS")
print("=" * 60)
print(f"  Company:            Dynamic Air Engineering India Private Limited")
...
print(f"  Accuracy vs BCIPL ({bcipl_baseline}%):  {'+' if accuracy_pct >= bcipl_baseline else ''}{accuracy_pct - bcipl_baseline:.1f}pp  "
