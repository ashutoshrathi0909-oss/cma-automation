#!/usr/bin/env python3
"""
Kurunji Retail accuracy benchmark — Phase 1 only (classify + compare).

Run: docker compose exec worker python /app/test-results/run_accuracy_kurunji.py
Outputs:
  /app/test-results/Kurunji_Retail_accuracy_summary.json
  /app/test-results/Kurunji_Retail_wrong_entries.json
"""
...
# Ensure /app is on the path when run from a subdirectory
sys.path.insert(0, "/app")

# ─── Paths ────────────────────────────────────────────────────────────────────
GT_FILE    = Path("/app/CMA_Ground_Truth_v1/companies/Kurunji_Retail/ground_truth_normalized.json")
OUTPUT_DIR = Path("/app/test-results")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_FILE = OUTPUT_DIR / "Kurunji_Retail_accuracy_summary.json"
WRONG_FILE   = OUTPUT_DIR / "Kurunji_Retail_wrong_entries.json"

# ─── Cost guards ──────────────────────────────────────────────────────────────
MAX_ITEMS              = 500          # hard cap (Kurunji has 49, well under)
...
print("=" * 60)
print("Kurunji Retail Accuracy Benchmark")
print("=" * 60)
