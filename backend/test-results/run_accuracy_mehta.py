#!/usr/bin/env python3
"""
Mehta Computer accuracy benchmark — Phase 1 only (classify + compare).

Run: docker compose exec worker python /app/test-results/run_accuracy_mehta.py
Outputs:
  /app/test-results/Mehta_Computer_accuracy_summary.json
  /app/test-results/Mehta_Computer_wrong_entries.json
"""
...
# Ensure /app is on sys.path so `app.*` imports resolve when running from a subdirectory
sys.path.insert(0, "/app")

# ─── Paths ────────────────────────────────────────────────────────────────────
GT_FILE = Path("/app/CMA_Ground_Truth_v1/companies/Mehta_Computer/ground_truth_normalized.json")
OUTPUT_DIR = Path("/app/test-results")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_FILE = OUTPUT_DIR / "Mehta_Computer_accuracy_summary.json"
WRONG_FILE = OUTPUT_DIR / "Mehta_Computer_wrong_entries.json"
...
print("=" * 60)
print("Mehta Computer Accuracy Benchmark — Phase 1")
print("=" * 60)
...
print("RESULTS")
print("=" * 60)
print(f"  Company:            Mehta_Computer")
