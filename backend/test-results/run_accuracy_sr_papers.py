#!/usr/bin/env python3
"""
SR_Papers accuracy benchmark — Phase 1 only (classify + compare).

Run: docker compose exec worker python /app/test-results/run_accuracy_sr_papers.py
Outputs:
  /app/test-results/SR_Papers_accuracy_summary.json
  /app/test-results/SR_Papers_wrong_entries.json
"""
...
# Ensure /app is on sys.path so "from app.services..." imports work
sys.path.insert(0, "/app")

# ─── Paths ────────────────────────────────────────────────────────────────────
GT_FILE    = Path("/app/CMA_Ground_Truth_v1/companies/SR_Papers/ground_truth_normalized.json")
OUTPUT_DIR = Path("/app/test-results")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_FILE = OUTPUT_DIR / "SR_Papers_accuracy_summary.json"
WRONG_FILE   = OUTPUT_DIR / "SR_Papers_wrong_entries.json"

# ─── Cost guards ──────────────────────────────────────────────────────────────
MAX_ITEMS              = 500          # hard cap (SR_Papers has 164, well under)
...
print("=" * 60)
print("SR_Papers Accuracy Benchmark")
print("=" * 60)
